...
export type QueueElementBaseExtend = Required<QueueElementBase & {
    stateDesc: string
}>;
 ...
 */
export function useBigFileUpload(): [ProgressStateType, (uniqueCode: string) => void, (uniqueCode: string) => void] {
    // 添加收到消息的订阅事件
    emitterAndTaker.on(UPLOADING_FILE_SUBSCRIBE_DEFINE, function (el: QueueElementBaseExtend) {
        // 判断是否存在 用来判断是否要添加
