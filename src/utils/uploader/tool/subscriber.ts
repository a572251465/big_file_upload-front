...
emitterAndTaker.on(
    ...
    async function (uniqueCode: string, action: UploadProgressState) {
        ...
        if (equals(action, UploadProgressState.Canceled) && !globalProgressState.current.has(uniqueCode)) {
            Logger.warning(strFormat("[%s] 进度已经完成了, 可以自行进行删除.", uniqueCode));
            return;
        }
...
emitterAndTaker.on(UPLOADING_FILE_SUBSCRIBE_DEFINE, async function (el: QueueElementBase) {
    ...
    switch (el.type) {
        ...
        case UploadProgressState.QuickUpload:
            ...
            if (!isArray(pMethods)) {
                console.warn(strFormat("[%s(%s)] 无法获取到进度信息, 请联系开发者", fileName, el.uniqueCode!));
                return;
            }
            const [m1, , m3] = pMethods!;
            if (isFunction(m1))
                m1([calculationHashName, fileName]);
            if (isFunction(m3))
                m3([calculationHashName, fileName]);
            break;
    }
