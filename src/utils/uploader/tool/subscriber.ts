...
emitterAndTaker.on(
    ...
    async function (el: QueueElementBase) {
        ...
        /* 判断状态是否处于完成状态 */
        if ([UploadProgressState.Done, UploadProgressState.QuickUpload, UploadProgressState.RetryFailed, UploadProgressState.Canceled].includes(el.type!)) {
            /* 直接删除 */
        ...
        /* 判断是否处于运行状态 */
        if ([UploadProgressState.Uploading, UploadProgressState.BreakPointUpload, UploadProgressState.Merge, UploadProgressState.Pause, UploadProgressState.Retry].includes(el.type!)) {
            /* 汇集key */
            ...
            for (const [key, value] of mapInfo)
